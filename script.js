...
/* ---------- 4. INSTANT VALUE SET (no animation) ---------- */
function setKpiValue(el, raw, opts={}) {
  const { prefix='', suffix='', decimals=0, money=false } = opts;
  el.textContent = money ? fmtMoney(raw) : prefix + raw.toFixed(decimals) + suffix;
}
...
function initInteractions() {
  ...
  ['kpiGrid','invKpiGrid'].forEach(id=>{
    ...
    const ko = new IntersectionObserver((entries)=>{
      entries.forEach(e=>{ if(e.isIntersecting){ e.target.classList.add('in'); ko.unobserve(e.target);} });
    }, {threshold:.2});
    ko.observe(grid);
  });
